...
const statusOptions = ['已入库', '待付款', '待审核'];
...
const calcTotal = (items) => (Array.isArray(items) ? items.reduce((sum, item) => sum + calcItemSubtotal(item), 0) : 0);
...
function Purchase() {
  ...
  const watchedItems = useMemo(
    () => (Array.isArray(watchedItemsRaw) ? watchedItemsRaw : []),
    [watchedItemsRaw]
  );

  const supplierOptions = useMemo(
    () => (suppliers || []).map((item) => ({ label: item.name, value: item.name })).filter((item) => item.value),
    [suppliers]
  );
  ...
  const loadAll = async () => {
    ...
    setError('');
    try {
      const [purchaseData, productsData, suppliersData] = await Promise.all([
        fetchResource('purchases'),
        fetchResource('products'),
        fetchResource('suppliers'),
      ]);
      setOrders(purchaseData.items || []);
      setProducts(productsData.items || []);
      setSuppliers(suppliersData.items || []);
    } catch (err) {
      setError(err?.response?.data?.error || '采购数据加载失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      await deleteResource('purchases', id);
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSubmitting(true);
      if (editingRecord?.id) {
        await updateResource('purchases', editingRecord.id, payload);
        message.success('采购订单更新成功');
      } else {
        await createResource('purchases', payload);
    ...
    { title: '日期', dataIndex: 'date', key: 'date', width: 130 },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm title="确认删除该采购订单？" okText="删除" cancelText="取消" onConfirm={() => handleDelete(record.id)}>
    ...
    <>
      ...
      <Card className="surface-card">
        ...
        <Table
          ...
          scroll={{ x: 980 }}
          locale={{ emptyText: '暂无采购订单，请点击“新增采购”' }}
        />
      </Card>
      ...
      >
        <Form form={form} layout="vertical">
          <Space size={12} style={{ width: '100%' }} align="start">
            ...
            >
              <Input placeholder="例如：PO-202604-001" />
            </Form.Item>
            <Form.Item
              label="供应商"
              name="supplier"
            ...
            >
              <Select
                ...
                notFoundContent="暂无供应商，请先到供应商管理新增"
              />
            </Form.Item>
          </Space>
            ...
            {(fields, { add, remove }) => (
              <>
                ...
                {fields.map((field, index) => {
                  ...
                  const subtotal = calcItemSubtotal(row);
                  return (
                    <Space
                      key={field.key}
                      align="baseline"
                    ...
                    >
                      ...
                      >
                        <Select
                          ...
                          notFoundContent="暂无商品，请先到商品管理新增"
                        />
                      </Form.Item>
                      ...
                      <Form.Item label={index === 0 ? '小计' : ''} style={{ width: 120 }}>
                        <Input value={Number(subtotal || 0).toLocaleString()} readOnly />
                      </Form.Item>
