...
const statusOptions = ['已完成', '待收款', '待发货'];
...
function SalesOrder() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setOrders(salesData.items || []);
      setProducts(productsData.items || []);
      setCustomers(customersData.items || []);
    } catch (err) {
      setError(err?.response?.data?.error || '销售订单数据加载失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSubmitting(true);
      if (editingRecord?.id) {
        await updateResource('sales', editingRecord.id, payload);
        message.success('销售订单更新成功');
      } else {
        await createResource('sales', payload);
    ...
    { title: '日期', dataIndex: 'date', key: 'date', width: 130 },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Popconfirm title="确认删除该销售订单？" okText="删除" cancelText="取消" onConfirm={() => handleDelete(record.id)}>
    ...
    <>
      ...
      <Card className="surface-card">
        ...
        <Table
          ...
          scroll={{ x: 980 }}
          locale={{ emptyText: '暂无销售订单，请点击“新增订单”' }}
        />
      </Card>
      ...
      >
        <Form form={form} layout="vertical">
          <Space size={12} style={{ width: '100%' }} align="start">
            ...
            >
              <Input placeholder="例如：SO-202604-001" />
            </Form.Item>
            <Form.Item
              label="客户"
              name="customer"
            ...
            >
              <Select
                ...
                notFoundContent="暂无客户，请先到客户管理新增"
              />
            </Form.Item>
          </Space>
